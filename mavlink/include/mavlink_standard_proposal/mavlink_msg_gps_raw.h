 ...
 */
static inline uint16_t mavlink_msg_gps_raw_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg, float lat, float lon, float alt, float eph, float epv, float v, float hdg, uint8_t fix_type)
{
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_gps_raw_pack_chan(uint8_t system_id, uint8_t component_id, uint8_t chan, mavlink_message_t* msg, float lat, float lon, float alt, float eph, float epv, float v, float hdg, uint8_t fix_type)
{
	mavlink_gps_raw_t *p = (mavlink_gps_raw_t *)&msg->payload[0];
	msg->msgid = MAVLINK_MSG_ID_GPS_RAW;
...
{
	return mavlink_msg_gps_raw_pack(system_id, component_id, msg, gps_raw->lat, gps_raw->lon, gps_raw->alt, gps_raw->eph, gps_raw->epv, gps_raw->v, gps_raw->hdg, gps_raw->fix_type);
}
...
static inline void mavlink_msg_gps_raw_send(mavlink_channel_t chan, float lat, float lon, float alt, float eph, float epv, float v, float hdg, uint8_t fix_type)
{
