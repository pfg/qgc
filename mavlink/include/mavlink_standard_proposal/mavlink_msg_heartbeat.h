...
#define MAVLINK_MSG_ID_HEARTBEAT_LEN 11
#define MAVLINK_MSG_0_LEN 11
 ...
 */
static inline uint16_t mavlink_msg_heartbeat_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg, uint64_t time, uint8_t type, uint8_t autopilot)
{
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_heartbeat_pack_chan(uint8_t system_id, uint8_t component_id, uint8_t chan, mavlink_message_t* msg, uint64_t time, uint8_t type, uint8_t autopilot)
{
	mavlink_heartbeat_t *p = (mavlink_heartbeat_t *)&msg->payload[0];
	msg->msgid = MAVLINK_MSG_ID_HEARTBEAT;
...
{
	return mavlink_msg_heartbeat_pack(system_id, component_id, msg, heartbeat->time, heartbeat->type, heartbeat->autopilot);
}
...
static inline void mavlink_msg_heartbeat_send(mavlink_channel_t chan, uint64_t time, uint8_t type, uint8_t autopilot)
{
