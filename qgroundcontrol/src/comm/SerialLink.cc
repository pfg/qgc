...
void SerialLink::run()
{
    ...
    forever
    {
        ...
        {
            dataMutex.unlock();
            checkForBytes(); // Check if new bytes have arrived, if yes, emit the notification signal
        } else
        if (this->unlockReady)
            dataMutex.unlock(), this->unlockReady--;
        else
        {
...
void SerialLink::receiveFinished(void)
{
    this->unlockReady++;
}
